    ...
    <g:if test="${haveResults}">
      ...
      <div class="results">
        <g:each var="result" in="${searchResult.docs}" status="index">
        	...
        	&nbsp;-&nbsp;
        	<g:link controller="volume" action="index" params="${[volumeId:result.volumeId]}">Rate volume</g:link>
					 	     ...
					 	     <g:formRemote name="rating-${result.id}" url="[controller: 'freeSearch', action:'rate']"
					 	     	...
					 	     	<g:hiddenField name="docId" value="${result.id}"/>
						   		   <g:radioGroup name="rating" required="true"
              							...
              							values="['ITEM_TITLE','ITEM_INTERNAL', 'VOL_TITLE', 'VOL_TOC','VOL_INTERNAL','VOL_INDEX', 'OTHER']">
      ...
      <div>
        <div class="paging">
          <g:if test="${haveResults}">
              ...
              <g:else><g:paginate controller="freeSearch" action="index" params="[q: params.q]" total="${searchResult.numFound}" prev="&lt; previous" next="next &gt;"/></g:else>
          </g:if>
        </div>
      </div>
    </g:if>
