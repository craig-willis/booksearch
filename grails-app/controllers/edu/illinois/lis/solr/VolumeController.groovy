...
class VolumeController {
    ...
    def index() { 
		if (!params.volumeId?.trim()) {
			return [:]
		}
		
		def volumeId = params.volumeId
		...
		// volumeId:"loc.ark:/13960/t53f5fh9x"
		withHttp(uri: solrUrl) {
			def json = get(path: "/solr/htrc/select/", query: [q: "volumeId:\"" + volumeId + "\"", wt: "json", start: offset, rows: max, sort: "pageNum asc"])
